...
{
	const tableBody =  tableEle.getElementsByTagName('tbody')[0];
	...
	for(i = 0; i < reimbsList.length; i++)
	{
		...
		const nameText = document.createTextNode(reimb.author_firstname + " " + reimb.author_lastname);
		...
		if(reimb.status == 'PENDING')
		{
			...
			viewBtn.innerText = "Resolve";
			
			//Append to form
			form.appendChild(inputType);
			form.appendChild(viewBtn);
			
			form.setAttribute('action', 'resolve');
		}
		else
		{
			const viewBtn = document.createElement('button');
...
function getReimb()
{
	...
	xhr.open('GET', "api/getAllReimbursements", true);
